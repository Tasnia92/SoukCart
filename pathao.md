# Developer API | Merchant Panel | Pathao
...
![Wordpress](/assets/wordpress.a787ecf6.svg)

Wordpress Plugin

[View >](https://github.com/pathao-eng/courier-woocommerce-plugin)

![Shopify](/assets/shopify.5c1a03a3.svg)

Shopify Plugin

[View >](https://apps.shopify.com/pathao-courier)
...
You can see client\_id from merchant api credentials section.

client\_secret

You can see client\_secret from merchant api credentials section.
...
Must use grant type refresh\_token for refresh token api.
...
Provide your refresh token in order to generate access\_token
...
Name of the store. Store name length should be between **3** to **50** characters.
...
Contact person of the store need for issue related communication. Contact person name length should be between **3** to **50** characters.
...
Store contact person phone number. Contact number length should be **11** characters.
...
Store contact person secondary phone number. Secondary contact number length should be **11** characters. This field is optional.
...
OTP for orders from this order will be sent to this number
...
Merchant Store address. Address length should be between **15** to **120** characters.
...
The name of the store that you created.
...
store\_id is provided by the merchant and not changeable. This store ID will set the pickup location of the order according to the location of the store.
...
Parcel receiver’s city\_id. This is an optional parameter, so do not send a null value. If not included in the request payload, then our system will populate it automatically based on the recipient\_address you will be provided.
...
Parcel receiver’s zone\_id. This is an optional parameter, so do not send a null value. If not included in the request payload, then our system will populate it automatically based on the recipient\_address you will be provided.
...
Parcel receiver’s area\_id. This is an optional parameter. If not included in the request payload, then our system will populate it automatically based on the recipient\_address you will be provided.
...
An array of order objects is required to send within the request body.
...
Http response code for bulk order creation.
...
Data field is true if bulk order creation is accepted.
...
1 if the store is default\_store otherwise 0.

is\_default\_return\_store

integer

No

1 if the store is default\_return\_store otherwise 0.
