...
begin
  ...
  select
    ...
    coalesce(count(*) filter (where product.is_active and product.stock > 0 and product.stock <= v_low_stock_threshold), 0)::integer,
    coalesce(count(*) filter (where product.is_active and product.stock <= 0), 0)::integer,
    coalesce(count(*) filter (where product.is_active and product.stock > v_low_stock_threshold), 0)::integer
  ...
  from (
    ...
    where product.seller_id = v_seller_id
      ...
      and product.stock <= v_low_stock_threshold
