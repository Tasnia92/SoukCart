...
describe("retailer catalog API", () => {
  ...
  it("resolves the next cart quantity within stock", () => {
    expect(nextCartQuantity({ name: "Atlas dates", stock: 10, min_order_qty: 1 }, 2, 3)).toBe(5);
    expect(nextCartQuantity({ name: "Atlas dates", stock: 5, min_order_qty: 1 }, 0, 5)).toBe(5);
    expect(nextCartQuantity({ name: "Rice sack", stock: 50, min_order_qty: 10 }, 0, 1)).toBe(10);
  });

  it("reports remaining stock with correct pluralization", () => {
    expect(() =>
      nextCartQuantity({ name: "Atlas dates", stock: 5, min_order_qty: 1 }, 3, 3),
    ).toThrow("Only 2 more units of Atlas dates are in stock.");
    expect(() =>
      nextCartQuantity({ name: "Atlas dates", stock: 5, min_order_qty: 1 }, 4, 2),
    ).toThrow("Only 1 more unit of Atlas dates are in stock.");
  });

  it("blocks adding a product from a second supplier", () => {
    expect(
      cartSupplierConflict(product({ seller_id: "seller-2", seller_name: "Other" }), [
        product({ seller_id: "seller-1", seller_name: "Samira" }),
      ]),
    ).toBe("Your cart is from Samira. Checkout one supplier at a time.");
    expect(
      cartSupplierConflict(product({ seller_id: "seller-1" }), [
        product({ id: "a", seller_id: "seller-1" }),
      ]),
    ).toBeNull();
  });
  ...
  it("reports when all stock is already in the order", () => {
    expect(() =>
      nextCartQuantity({ name: "Atlas dates", stock: 5, min_order_qty: 1 }, 5, 1),
    ).toThrow("You already have all 5 units of Atlas dates in your order.");
    expect(() => nextCartQuantity({ name: "Single", stock: 1, min_order_qty: 1 }, 1, 1)).toThrow(
